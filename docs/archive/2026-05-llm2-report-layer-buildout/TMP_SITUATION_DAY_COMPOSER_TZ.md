...
## Step DDC-10 — Полировка первого менеджерского отчета

Статус: в реализации 2026-05-19.

Цель: убрать из первого manager-facing отчета блоки/колонки, которые мешают восприятию, и добавить недостающий контекст по количеству звонков в `БАЛЛЫ ПО ЭТАПАМ`.

Требования:

- `Разбор звонка`:
  - колонка `Суть момента` переименовывается в `Подтверждение из звонка`;
  - смысловой комментарий/объяснение переносится в `Что было`;
  - в `Подтверждение из звонка` остается только фактический фрагмент разговора / мини-диалог;
  - если подтверждающего фрагмента нет, не подменять его авторским выводом.
- `ЧЕЛЛЕНДЖ НА ЗАВТРА`:
  - временно полностью скрыть из manager-facing отчета;
  - payload можно оставить, чтобы не ломать downstream, но renderers не должны выводить блок.
- `БАЛЛЫ ПО ЭТАПАМ`:
  - добавить колонку `Звонков`;
  - значение = количество звонков, где этап проявился и участвовал в расчете stage score.

Проверка:

- `py_compile` для `reporting.py` и `report_templates.py`;
