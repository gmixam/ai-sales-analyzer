...
- `docs/PILOT40_ONLINEPBX_ALL_UPSTREAM_TZ.md` - реализованное мини-ТЗ по
  live upstream без Bitrix scope dependency: транскрибация/LLM1 берут все
  OnlinePBX CDR, а analysis/reporting остаются в selected ЭДО manager scope.
...
| PILOT-40 | OnlinePBX-all upstream without Bitrix scope | `implemented_first_pass_live_enabled` | ТЗ: [`docs/PILOT40_ONLINEPBX_ALL_UPSTREAM_TZ.md`](PILOT40_ONLINEPBX_ALL_UPSTREAM_TZ.md). Добавлен upstream режим `onlinepbx_all`: scheduled scope не строит manager_ids/extensions из Manager/Bitrix, `CallProcessingService` targets all OnlinePBX CDR after technical duration filters, forecast budget gate срабатывает до recording URL/STT/LLM1, intake пропускает Bitrix fallback и сохраняет unknown manager as `manager_id=null` with fallback department. Live `.env.call-processing` переключен на `onlinepbx_all`, provider budget `700`; downstream analysis остается только ЭДО manager_daily allowlist | Проверено: `py_compile`, `git diff --check`, docker focused pytest `23 passed, 27 deselected`; live env check OK; dry-run `2026-06-29`: `362` CDR targeted, `150` eligible audio, `212` no-audio/zero-talk/missed, `provider_calls_estimate=451/700`, provider calls made `0`, blockers `[]` |
