...
| 2026-06-14-split-12jun-02 | call-processing split cleanup | `new` | При первом split ensure найден write-path дефект active artifacts: pending/active duplicate для `(interaction_id, artifact_kind, artifact_version)` мог падать на `uq_call_artifacts_active_kind_version`. Код исправлен в `ArtifactRepository.write_active()` и покрыт focused тестом. Остаток: две ранние упавшие попытки ensure остались в `call_core.call_processing_runs` как `running`, хотя процессов нет; нужен безопасный cleanup stale runs/status repair, чтобы не сбивать будущие cost/readiness/audit запросы. |
