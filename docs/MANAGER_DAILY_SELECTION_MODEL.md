...
- `report_evidence` contains report-ready candidates for `business_outcome`, `СИТУАЦИЯ ДНЯ`, `РАЗБОР ЗВОНКА`, `ГОЛОС КЛИЕНТА`, `ДЕНЬГИ НА СТОЛЕ`, `КОГО ВЗЯТЬ В РАБОТУ ЗАВТРА`, `ЧЕЛЛЕНДЖ НА ЗАВТРА`, `СПИСОК ВСЕХ ЗВОНКОВ ДНЯ` context, legacy `ДОПОЛНИТЕЛЬНЫЕ СИТУАЦИИ`, and reusable quote bank.
