...
Последняя договоренность по production pilot scope:

- 2026-06-15 выполнен live Bitrix sync по `[ЭДО] Отдел Продаж`;
- для schedule/дневных прогонов берем всех актуальных рабочих кандидатов:
  Алишер, Илья, Тимур, Толеген;
- inactive/уволенные сотрудники не считаются;
- `Робот Договор24` исключен как технический пользователь;
- конкретные `manager_ids` зафиксированы в `docs/PILOT_OPERATIONS.md`.
