...
### 5.0 Язык документации

Вся проектная документация должна вестись на русском языке.

Правило применяется ко всем новым и изменяемым документам, включая `docs/`,
`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, task-шаблоны, roadmap, progress,
decisions и журналы обратной связи.

Допустимые исключения:
- имена файлов, классов, функций, полей JSON, enum/status keys, команд,
  route/path, Git branch/commit ids и другие технические идентификаторы;
- прямые цитаты из внешних источников или пользовательской обратной связи;
- названия моделей, сервисов, API и provider-specific терминов;
- короткие технические метки, которые должны совпадать с кодом или payload.

Если в существующем документе уже есть англоязычные фрагменты, кодер не обязан
переводить весь файл в рамках несвязанной задачи. Но любой новый или изменяемый
человекочитаемый текст в документации должен быть написан на русском языке.

### 5.1 Проверка влияния на документацию
...
### 5.2 Когда обязательно обновлять `DECISIONS.md`
...
### 5.3 Когда обязательно обновлять `PROGRESS.md`
