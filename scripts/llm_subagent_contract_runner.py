#!/usr/bin/env python3
"""Deterministic repository runner for AI_LLM_SUBAGENT_RUNNER_CMD.

The runner intentionally replaces only the model boundary. It reads one JSON
request from stdin or AI_LLM_SUBAGENT_INPUT_PATH, returns one JSON object on
stdout, and writes the same object to AI_LLM_SUBAGENT_OUTPUT_PATH when provided.
"""
...
CORE_ROOT = REPO_ROOT / "core" if (REPO_ROOT / "core" / "app").exists() else REPO_ROOT
