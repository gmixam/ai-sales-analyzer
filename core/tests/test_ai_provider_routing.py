...
class AIProviderRoutingTests(unittest.TestCase):
    ...
    def test_llm2_layered_analysis_mode_fails_closed_on_pass_failure(self) -> None:
        ...
        self.assertEqual(forensics["normalized_result"], {})
    ...
    def test_llm2_layered_runtime_uses_compact_common_prompt_not_architecture_contract(self) -> None:
        analyzer = CallsAnalyzer(department_id=str(uuid4()), db=None)
        interaction = SimpleNamespace(
            id=uuid4(),
            metadata_={},
            text="Client asked for materials.",
        )
        prompts_dir = CORE_ROOT / "app" / "agents" / "calls" / "prompts"
        common_prompt = (prompts_dir / "llm2_common_runtime.md").read_text(encoding="utf-8")
        architecture_contract = (prompts_dir / "llm2_pass_contracts.md").read_text(
            encoding="utf-8"
        )
        ...
        with patch.object(
            analyzer,
            "_request_llm_content",
            return_value='{"pass":"LLM-2A","scenes":[],"evidence_ledger":[]}',
        ) as request_mock:
            analyzer._request_llm2_layered_pass(
                interaction=interaction,
                instruction_version=APPROVED_INSTRUCTION_VERSION,
                request_kind="llm2a_facts_scenes",
                system_prompt="Node-specific runtime prompt.",
                prompt_context={
                    "llm1_first_pass": {},
                    "checklist_definition": {},
                },
        ...
        system_prompt = request_mock.call_args.kwargs["messages"][0]["content"]
        self.assertLess(len(common_prompt), len(architecture_contract))
        self.assertIn("LLM2 Common Runtime Prompt", system_prompt)
        self.assertIn("Return JSON only", system_prompt)
        self.assertIn("transcript -> scenes -> evidence -> claim -> proof_card", system_prompt)
        self.assertIn("Node-specific runtime prompt.", system_prompt)
        self.assertIn("Status: handoff-ready contract asset", architecture_contract)
        self.assertNotIn("LLM2 Pass Contracts 2A/2B/2C/2D", system_prompt)
        self.assertNotIn("## LLM-2A: Facts / Scenes / Evidence Ledger", system_prompt)
        self.assertNotIn("Required output:", system_prompt)
