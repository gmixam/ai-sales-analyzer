...
RUN_ALERT_REASON_LABELS = {
    ...
    "skip_accumulate_readiness_not_met": "отчет требует review или накопления данных",
    "analysis_missing": "нет готового анализа",
    "transcript_missing": "нет готового транскрипта",
}
...
def _bounded_lines(lines: list[str], *, max_items: int, omitted_label: str) -> list[str]:
    ...
    visible = compact_lines[:max_items]
    omitted = max(len(compact_lines) - len(visible), 0)
    if omitted:
        visible.append(f"Еще {omitted} {omitted_label} см. в observability/logs.")
    return visible


def _safe_operator_text(value: Any, *, max_len: int = 96) -> str:
...
def _operator_reason_label(reason_code: str) -> str:
    code = _safe_operator_text(reason_code, max_len=80)
    if not code:
        return ""
    normalized = code.strip().lower()
    if normalized in RUN_ALERT_REASON_LABELS:
        return f"{code}: {RUN_ALERT_REASON_LABELS[normalized]}"
    prefix = normalized.split(":", 1)[0]
    if prefix in RUN_ALERT_REASON_LABELS:
        return f"{code}: {RUN_ALERT_REASON_LABELS[prefix]}"
    if "email" in normalized and ("failed" in normalized or "timeout" in normalized):
        return f"{code}: ошибка отправки email"
    if "timeout" in normalized or "readtimeout" in normalized:
        return f"{code}: таймаут при ожидании сервиса"
    if "quota" in normalized or "budget" in normalized:
        return f"{code}: ограничение бюджета или лимита"
    if "analysis" in normalized and ("missing" in normalized or "failed" in normalized):
        return f"{code}: нет готового анализа или анализ завершился ошибкой"
    return code


def _run_alert_impact_text(*, overall_status: str, report_statuses: list[str]) -> str:
    statuses = set(report_statuses)
    if overall_status in {"blocked", "failed", "no_data"} or statuses & {"blocked", "missing_artifacts"}:
        return "Часть менеджеров может не получить ежедневный отчет."
    if overall_status == "partial" or statuses & {"partial", "skip_accumulate"}:
        return "Часть менеджеров может не получить отчет; часть отчетов требует review."
    if "review_required" in statuses:
        return "Отчет требует review перед обычной отправкой менеджеру."
    return "Прогон завершен, оператору стоит проверить итоговую доставку."
...
class CallsManualReportingOrchestrator:
    ...
    async def _ensure_call_processing_source_artifacts(
        ...
        *,
        ...
        source_targeted_total = int(planned.get("source_targeted_total") or interactions_total)
        source_persisted_total = int(
            planned.get("source_ingest_created") or 0
        ) + int(planned.get("source_ingest_skipped") or 0)
        if source_persisted_total <= 0:
            source_persisted_total = source_targeted_total
    ...
    def _send_run_monitor_alerts(
        ...
        *,
        ...
        for spec in specs:
            ...
            attempt = send_run_alert(spec["event"], **alert_kwargs)
            alert = {
                "kind": spec["kind"],
    ...
    def _build_run_monitor_alert_specs(
        ...
        *,
        ...
        reason_counter = self._collect_run_alert_reason_counter(
            reports=reports,
            errors=errors,
        )
        status_counts = {
        ...
        run_id = f"manual-report:{preset.code}:{period.get('date_from')}:{period.get('date_to')}"
        operator_summary = self._build_run_monitor_operator_summary(
            period=period,
            overall_status=overall_status,
    ...
    @staticmethod
    def _build_run_monitor_operator_summary(
        *,
        period: dict[str, str],
        overall_status: str,
        ...
        """Build a short human-facing manager_daily run monitor summary."""
        date_from = _safe_operator_text(period.get("date_from"), max_len=24) or "unknown"
        date_to = _safe_operator_text(period.get("date_to"), max_len=24) or "unknown"
        period_label = date_from if date_from == date_to else f"{date_from}..{date_to}"
        status_label = _safe_operator_text(overall_status, max_len=40) or "unknown"
        total_reports = sum(max(value, 0) for value in status_counts.values())
        count_lines = [
        ...
        for reason, count in reason_counter.most_common(5):
            label = _operator_reason_label(reason)
            if not label:
                continue
            suffix = f" ({count})" if count > 1 else ""
            top_reason_lines.append(f"- {label}{suffix}")
        top_reason_lines = _bounded_lines(
            top_reason_lines or ["- none"],
            max_items=5,
            omitted_label="причин",
        )
        impact = _run_alert_impact_text(
            overall_status=overall_status,
            report_statuses=report_statuses,
        )
        lines = [
            f"Ежедневные отчеты менеджеров за {period_label}: {status_label}",
            "",
            "Что случилось:",
            f"Период: {period_label}. Общий статус: {status_label}.",
            "",
            "Показатели:",
            *_bounded_lines(count_lines, max_items=6, omitted_label="статусов"),
            "",
            "Основные причины:",
            *top_reason_lines,
            "",
            "Что затронуто:",
            f"- manager_daily reports: {total_reports}",
            "",
            "На что влияет:",
            impact,
            "",
            "Что проверить:",
            "scheduled_report_batches / drafts / observability; delivery result.",
            "",
            f"Run: {run_id}",
        ]
        return "\n".join(lines)
    ...
    @staticmethod
    def _collect_run_alert_reason_counter(
        *,
        reports: list[dict[str, Any]],
        errors: list[str],
    ) -> Counter[str]:
        """Count alert reasons for operator-facing top reason summaries."""
        counter: Counter[str] = Counter()
        for item in errors:
            code = _safe_operator_text(item)
            if code:
                counter[code] += 1
        for report in reports:
            for item in report.get("readiness_reason_codes") or []:
    ...
    def _render_and_deliver_report_result(
        ...
        *,
        ...
        if preset.code == "manager_daily":
            ...
            _localize_manager_daily_payload(payload)
...
def _focus_case_decision(
    *,
    daily_focus: dict[str, Any] | None,
    case_status: str,
    selected_call_id: str | None,
    selected_case_stage_code: str | None,
    ...
    return {
        "focus_stage_code": focus_stage or None,
        "focus_stage_source": str((daily_focus or {}).get("challenge_metric_source") or "").strip()
        or "score_by_stage.priority",
        "case_status": case_status,
        "selected_call_id": selected_call_id or None,
        "selected_case_stage_code": selected_case_stage_code or None,
...
def _attach_focus_case_diagnostics(
    *,
    block: dict[str, Any] | None,
    decision: dict[str, Any] | None,
) -> dict[str, Any] | None:
    if not block or not decision:
        return block
    updated = dict(block)
    for key in (
        "case_status",
        ...
        "rejection_reasons",
        "focus_stage_code",
        "selected_case_stage_code",
    ):
        updated[key] = decision.get(key)
    diagnostics = dict(updated.get("selection_diagnostics") or {})
    diagnostics.update(
        {
            "case_status": decision.get("case_status"),
            "evidence_level": decision.get("evidence_level"),
            "wording_mode": decision.get("wording_mode"),
            "selection_reason": decision.get("selection_reason"),
            "rejection_reasons": decision.get("rejection_reasons") or [],
            "focus_stage_code": decision.get("focus_stage_code"),
            "selected_case_stage_code": decision.get("selected_case_stage_code"),
            "focus_case_selection": decision,
        }
    )
    updated["selection_diagnostics"] = diagnostics
    return updated


def _focus_case_selection_from_view(
    situation_day_coaching_view: dict[str, Any] | None,
) -> dict[str, Any] | None:
    diagnostics = (
        dict((situation_day_coaching_view or {}).get("selection_diagnostics") or {})
        if isinstance((situation_day_coaching_view or {}).get("selection_diagnostics"), dict)
        else {}
    )
    decision = diagnostics.get("focus_case_selection")
    return dict(decision) if isinstance(decision, dict) else None


def _focus_case_is_visible(decision: dict[str, Any] | None) -> bool:
    return str((decision or {}).get("case_status") or "").strip() in {"strong", "workable"}


def _call_breakdown_is_llm3_owned(call_breakdown: dict[str, Any] | None) -> bool:
    if not call_breakdown:
        return False
    source = str((call_breakdown or {}).get("call_breakdown_source") or "").strip()
    diagnostics = dict((call_breakdown or {}).get("selection_diagnostics") or {})
    quality = dict((call_breakdown or {}).get("call_breakdown_quality") or {})
    if diagnostics.get("deterministic_fallback_used") is True:
        return False
    if diagnostics.get("llm3_daily_call_breakdown") is True:
        return True
    if diagnostics.get("llm3_used") is True:
        return True
    if quality.get("selection_basis") == "verified_situation_day_llm3":
        return True
    return source in {"llm3_daily_situation.call_breakdown"}


def _call_breakdown_matches_focus_case(
    *,
    call_breakdown: dict[str, Any] | None,
    decision: dict[str, Any] | None,
    daily_focus: dict[str, Any] | None,
) -> bool:
    if not call_breakdown or not _focus_case_is_visible(decision):
        return False
    selected_call_id = str((decision or {}).get("selected_call_id") or "").strip()
    block_call_id = str((call_breakdown or {}).get("call_id") or "").strip()
    if not selected_call_id or block_call_id != selected_call_id:
        return False
    focus_stage = _daily_focus_stage_code(daily_focus)
    selected_stage = str((decision or {}).get("selected_case_stage_code") or "").strip()
    block_stage = str((call_breakdown or {}).get("stage_code") or "").strip()
    expected_stage = focus_stage or selected_stage
    if expected_stage and block_stage and block_stage != expected_stage:
        return False
    return True


def _situation_case_status_from_packet(
    *,
    packet: dict[str, Any] | None,
    view: dict[str, Any] | None,
) -> tuple[str, str, str]:
    proof_type = str((packet or {}).get("proof_type") or (view or {}).get("proof_type") or "").strip()
    proof_strength = str((packet or {}).get("proof_strength") or (view or {}).get("proof_strength") or "").strip()
    if proof_type == "direct_gap" and proof_strength == "strong":
        return "strong", "strong", "confident"
    return "workable", "workable", "cautious"


def _normalize_focus_case_decision_from_llm3(
    *,
    daily_focus: dict[str, Any] | None,
    result: dict[str, Any],
    fallback_selected_call_id: str | None = None,
    fallback_stage_code: str | None = None,
) -> dict[str, Any] | None:
    """Return the LLM3-owned focus-case decision if the contract is present."""
    raw_decision = result.get("focus_case_selection")
    decision_source = dict(raw_decision) if isinstance(raw_decision, dict) else {}
    case_status = str(
        decision_source.get("case_status")
        or result.get("case_status")
        or ""
    ).strip()
    if not case_status:
        return None
    ...
    focus_stage = (
        str(decision_source.get("focus_stage_code") or "").strip()
        or _daily_focus_stage_code(daily_focus)
        or None
    )
    selected_call_id = (
        str(
            decision_source.get("selected_call_id")
            or result.get("selected_call_id")
            or fallback_selected_call_id
            or ""
        ).strip()
        or None
    )
    selected_case_stage_code = (
        str(
            decision_source.get("selected_case_stage_code")
            or result.get("selected_case_stage_code")
            or result.get("stage_code")
            or fallback_stage_code
            or ""
        ).strip()
        or None
    )
    evidence_level = str(
        decision_source.get("evidence_level")
        or result.get("evidence_level")
        or ("weak" if case_status in {"weak_blocked", "blocked_mismatch"} else case_status)
    ).strip()
    wording_mode = str(
        decision_source.get("wording_mode")
        or result.get("wording_mode")
        or ("blocked" if case_status in {"weak_blocked", "blocked_mismatch"} else "cautious")
    ).strip()
    rejection_reasons_raw = (
        decision_source.get("rejection_reasons")
        if isinstance(decision_source.get("rejection_reasons"), list)
        else result.get("rejection_reasons")
    )
    rejection_reasons = [
        str(item).strip()
        for item in (rejection_reasons_raw or [])
        if str(item or "").strip()
    ]
    return {
        "focus_stage_code": focus_stage,
        "focus_stage_source": str(
            decision_source.get("focus_stage_source")
            or (daily_focus or {}).get("challenge_metric_source")
            or "score_by_stage.priority"
        ).strip(),
        "case_status": case_status,
        ...
        "selection_reason": str(
            decision_source.get("selection_reason")
            or result.get("selection_reason")
            or ""
        ).strip()
        or None,
        "evidence_level": evidence_level or None,
        "wording_mode": wording_mode or None,
        "rejection_reasons": rejection_reasons,
    }


def _focus_case_contract_rejection_reason(
    *,
    decision: dict[str, Any] | None,
    daily_focus: dict[str, Any] | None,
    known_call_ids: set[str],
) -> str | None:
    """Validate the LLM3 case-decision contract without choosing semantics."""
    if not isinstance(decision, dict):
        return "focus_case_selection_missing"
    case_status = str(decision.get("case_status") or "").strip()
    if case_status not in {"strong", "workable", "weak_blocked", "blocked_mismatch"}:
        return "focus_case_selection_case_status_invalid"
    if case_status in {"weak_blocked", "blocked_mismatch"}:
        return case_status

    focus_stage = _daily_focus_stage_code(daily_focus)
    selected_stage = str(decision.get("selected_case_stage_code") or "").strip()
    if not selected_stage:
        return "selected_case_stage_code_missing"
    if focus_stage and selected_stage != focus_stage:
        return f"situation_stage_mismatch:{selected_stage}"

    selected_call_id = str(decision.get("selected_call_id") or "").strip()
    if not selected_call_id:
        return "selected_call_id_missing"
    if selected_call_id not in known_call_ids:
        return "selected_call_id_not_in_report_artifacts"
    return None


def _situation_selected_call_id(
    *,
    ...
    for value in (
        ...
        if isinstance((situation_day_evidence_packet or {}).get("dialogue_excerpt"), dict)
        else None,
        (situation_evidence_quote or {}).get("call_id"),
        (situation_day_coaching_view or {}).get("call_id"),
        dict((situation_day_coaching_view or {}).get("selection_diagnostics") or {}).get("selected_call_id"),
    ):
        text = str(value or "").strip()
        if text:
            return text
...
def _finalize_daily_coaching_focus(
    *,
    ...
    if not isinstance(focus_case_selection, dict):
        case_status = "blocked_mismatch" if (
            focus_stage and situation_stage and situation_stage != focus_stage
        ) else "weak_blocked"
        evidence_level = "weak"
        wording_mode = "blocked"
        selected_call_id = None
        selected_case_stage = situation_stage or None
        selection_reason = (
            f"situation_stage_mismatch:{situation_stage}"
            if case_status == "blocked_mismatch"
            else "focus_case_selection_missing"
        )
        rejection_reasons = [selection_reason, *list(issues)]
        focus_case_selection = _focus_case_decision(
            daily_focus=result,
            case_status=case_status,
            selected_call_id=selected_call_id,
            selected_case_stage_code=selected_case_stage,
            selection_reason=selection_reason,
            evidence_level=evidence_level,
            wording_mode=wording_mode,
            rejection_reasons=rejection_reasons,
        )
    elif not focus_case_selection.get("focus_stage_code"):
        focus_case_selection = {
            **focus_case_selection,
            "focus_stage_code": focus_stage or None,
            "focus_stage_source": str(result.get("challenge_metric_source") or "").strip()
            or "score_by_stage.priority",
        }
    result["focus_case_selection"] = focus_case_selection
    blockers = [
        issue
        for issue in issues
        if issue.startswith("call_breakdown_stage_mismatch:")
        or issue.startswith("situation_stage_mismatch:")
    ]
    case_status = str((focus_case_selection or {}).get("case_status") or "").strip()
    if case_status in {"weak_blocked", "blocked_mismatch"}:
        reason = str((focus_case_selection or {}).get("selection_reason") or case_status).strip()
        if reason and reason not in blockers:
            blockers.append(reason)
    result["validation"] = {
        "status": "blocked" if blockers else "warning" if issues else "passed",
        "issues": issues,
...
def _maybe_override_daily_focus_from_situation(
    *,
    ...
    """Keep the aggregate daily focus stable for PILOT-32 routing.

    Older pilot behavior allowed a selected Situation Day case to replace the
    score-derived daily focus. PILOT-32 forbids that substitution: downstream
    blocks must align to daily_focus.stage_code or become diagnostics/blockers.
    """
    return daily_focus
    ...
    *,
    ...
    for row in call_list:
        ...
        if selected_call_id and row_call_id == selected_call_id:
            continue
        status = str(_final_manager_status_from_call_row(row) or "").strip().lower()
...
def _additional_dialogue_from_item(item: dict[str, Any], evidence_quote: str) -> list[dict[str, str]]:
    """Return normalized evidence turns for a visible additional situation."""
    result: list[dict[str, str]] = []
    for raw in item.get("evidence_dialogue") or item.get("dialogue_evidence") or []:
        if isinstance(raw, dict):
            text = str(raw.get("text") or raw.get("quote") or "").strip()
            if not text:
                continue
            speaker = str(raw.get("speaker") or "unknown").strip().lower() or "unknown"
            result.append({"speaker": speaker, "text": text})
        else:
            text = str(raw or "").strip()
            if text:
                result.append({"speaker": "unknown", "text": text})
    return result or _additional_evidence_dialogue(evidence_quote)


def _extract_report_call_ids(value: Any) -> set[str]:
...
def _manager_daily_primary_block_call_ids(
    *,
    situation_evidence_quote: dict[str, Any] | None,
    situation_dialogue_excerpt: dict[str, Any] | None,
    situation_day_evidence_packet: dict[str, Any] | None,
    key_problem: dict[str, Any] | None,
    call_breakdown: dict[str, Any] | None,
    voice_of_customer: dict[str, Any] | None,
) -> set[str]:
    """Collect calls already used by Situation Day, Call Breakdown, or VoC."""
    result: set[str] = set()
    for source in (
        situation_evidence_quote,
        situation_dialogue_excerpt,
        situation_day_evidence_packet,
        dict(key_problem or {}).get("call_example"),
        call_breakdown,
    ):
        result.update(_extract_report_call_ids(source))
    for item in (voice_of_customer or {}).get("situations") or []:
        result.update(_extract_report_call_ids(item))
    for item in (voice_of_customer or {}).get("customer_scenes") or []:
        result.update(_extract_report_call_ids(item))
    return result
...
def _apply_additional_situations_quality_gate(
    *,
    ...
    excluded = {str(item).strip() for item in (excluded_call_ids or set()) if str(item or "").strip()}
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        evidence_dialogue = _additional_dialogue_from_item(item, evidence_quote)
        ...
        if not evidence_dialogue:
            reasons.append("missing_evidence")
        if str(item.get("evidence_quality") or "").strip().lower() == "weak":
            reasons.append("weak_evidence")
        item_call_ids = _extract_report_call_ids(item)
        if item_call_ids and excluded.intersection(item_call_ids):
            reasons.append("duplicate_primary_block_call")
        if kind == "gap" and (
...
def build_manager_daily_payload(
    *,
    ...
    report_block_routes = route_evidence_items(
        ...
        require_verified_proof=settings.ai_llm2_report_evidence_validation_enabled,
    )
    ...
    prebuilt_situation_day_packet = (
        dict((report_evidence_situation or {}).get("evidence_packet") or {})
        if isinstance((report_evidence_situation or {}).get("evidence_packet"), dict)
        else {}
    )
    if (
        prebuilt_situation_day_packet.get("status") == "verified"
        and prebuilt_situation_day_packet.get("case_status") in {"strong", "workable"}
    ):
        situation_day_evidence_packet = prebuilt_situation_day_packet
    else:
    ...
    if situation_day_evidence_packet is not None and situation_day_evidence_packet.get("status") == "verified":
        ...
        if situation_day_coaching_view is not None:
            case_status = str(situation_day_evidence_packet.get("case_status") or "").strip()
            situation_day_coaching_view = {
                **situation_day_coaching_view,
                "evidence_packet": situation_day_evidence_packet,
                "proof_strength": situation_day_evidence_packet.get("proof_strength"),
                "situation_day_evidence_status": "workable" if case_status == "workable" else "verified",
            ...
            if case_status:
                situation_day_coaching_view["case_status"] = case_status
                situation_day_coaching_view["evidence_level"] = situation_day_evidence_packet.get(
                    "evidence_level"
                ) or ("workable" if case_status == "workable" else "strong")
                situation_day_coaching_view["wording_mode"] = situation_day_evidence_packet.get(
                    "wording_mode"
                ) or ("cautious" if case_status == "workable" else "confident")
    ...
    focus_case_selection_for_breakdown = _focus_case_selection_from_view(situation_day_coaching_view)
    prebuilt_llm3_call_breakdown = (
        dict((report_evidence_situation or {}).get("call_breakdown") or {})
        if isinstance((report_evidence_situation or {}).get("call_breakdown"), dict)
        else None
    )
    if (
        prebuilt_llm3_call_breakdown is not None
        and _call_breakdown_is_llm3_owned(prebuilt_llm3_call_breakdown)
        and _call_breakdown_matches_focus_case(
            call_breakdown=prebuilt_llm3_call_breakdown,
            decision=focus_case_selection_for_breakdown,
            daily_focus=daily_coaching_focus,
        )
    ):
        call_breakdown = prebuilt_llm3_call_breakdown
    if (
        call_breakdown is None
        and
        situation_day_evidence_packet is not None
        and situation_day_evidence_packet.get("status") == "verified"
        and situation_call_id_for_breakdown
        and _focus_case_is_visible(focus_case_selection_for_breakdown)
    ):
        ...
        if (
            ...
            and _call_breakdown_is_llm3_owned(composer_breakdown)
            ...
            call_breakdown = None
    if call_breakdown is None and situation_call_id_for_breakdown:
        focus_stage_for_breakdown = _daily_focus_stage_code(daily_coaching_focus)
        routed_call_breakdown_items = [
            item
            for item in report_block_routes.get("call_breakdown") or []
            if str((item or {}).get("call_id") or "").strip() == situation_call_id_for_breakdown
            and (
                not focus_stage_for_breakdown
                or not str((item or {}).get("stage_code") or "").strip()
                or str((item or {}).get("stage_code") or "").strip() == focus_stage_for_breakdown
            )
        ]
        call_breakdown = _build_call_breakdown_from_evidence_registry_route(
            artifacts=coaching_content_artifacts,
            routed_items=routed_call_breakdown_items,
            score_by_stage=score_by_stage,
        )
        if not (
            _call_breakdown_is_llm3_owned(call_breakdown)
            and _call_breakdown_matches_focus_case(
                call_breakdown=call_breakdown,
                decision=focus_case_selection_for_breakdown,
                daily_focus=daily_coaching_focus,
    ...
    focus_case_selection_for_breakdown = (
        _focus_case_selection_from_view(situation_day_coaching_view)
    )
    if (
        isinstance(focus_case_selection_for_breakdown, dict)
        and call_breakdown
        and str((call_breakdown_quality or {}).get("status") or "") != "blocked_mismatch"
    ):
        call_breakdown = _attach_focus_case_diagnostics(
            block=call_breakdown,
            decision=focus_case_selection_for_breakdown,
        )
    ...
    additional_situations, additional_situations_quality = _apply_additional_situations_quality_gate(
        ...
        data_scope=coaching_data_scope,
        daily_focus=daily_coaching_focus,
        excluded_call_ids=_manager_daily_primary_block_call_ids(
            situation_evidence_quote=situation_evidence_quote,
            situation_dialogue_excerpt=situation_dialogue_excerpt,
            situation_day_evidence_packet=situation_day_evidence_packet,
            key_problem=key_problem,
...
_MANAGER_DAILY_EXACT_TEXT_TRANSLATIONS = {
    "The manager adapted the offer to the client's actual vehicle-sale document scenario and explained why the paid service was relevant for acceptance in СОН.": (
        "Менеджер адаптировал предложение под сценарий клиента с договором купли-продажи ТС и объяснил, зачем платная услуга нужна для принятия в ЦОН."
    ),
    "The manager fixed a concrete next step after the client postponed the decision.": (
        "Менеджер зафиксировал конкретный следующий шаг после того, как клиент отложил решение."
    ),
    "The manager kept the conversation constructive and did not pressure the client after the price comparison and hesitation.": (
        "Менеджер сохранил конструктивный тон и не давил после сравнения цены и сомнений клиента."
    ),
    "The manager established the basic relevance of the call by confirming the vehicle sale-purchase agreement, the client's private-person status, and the trigger for using Dogovor 24.": (
        "Менеджер подтвердил базовый контекст: договор купли-продажи ТС, статус клиента как физлица и причину обращения в Dogovor 24."
    ),
    "The opening did not clearly check whether the client was available or whether it was convenient to talk.": (
        "В начале разговора менеджер не проверил, удобно ли клиенту говорить."
    ),
    "At the start of an external client call, check whether the client can speak and adapt if needed.": (
        "В начале внешнего клиентского звонка сначала проверь, может ли клиент говорить, и подстройся под ситуацию."
    ),
    "The manager did not clarify the client's decision context after the client hesitated.": (
        "После сомнения клиента менеджер не уточнил, от чего зависит решение."
    ),
    "When a client says they will think, clarify what will determine the decision or whether anyone else is involved.": (
        "Когда клиент говорит, что подумает, уточни критерии решения и кто еще влияет на него."
    ),
    "The call ended open without clarifying urgency or fixing a concrete return point.": (
        "Звонок завершился открыто: срочность не уточнена, конкретная точка возврата не зафиксирована."
    ),
    "The manager answered the free-service question without first clarifying what exactly made the price difficult for the client.": (
        "Менеджер ответил на вопрос о бесплатном варианте, не уточнив, что именно делает цену сложной для клиента."
    ),
    "The current document situation was understood from the client's description, but the manager did not deeply explore the broader process.": (
        "Текущая ситуация с документом была понятна из слов клиента, но менеджер не углубился в более широкий процесс."
    ),
    "The client deferred the decision, but the manager did not clarify decision logic, who else influences it, or what would determine the choice.": (
        "Клиент отложил решение, но менеджер не уточнил логику принятия, участников и критерии выбора."
    ),
    "The basic logic was understandable, but the client still needed to clarify whether the already signed document had been sent anywhere and whether payment was required.": (
        "Базовая логика была понятна, но клиенту еще нужно было уточнить, был ли уже отправлен подписанный документ и нужна ли оплата."
    ),
    "After the notary-cost comparison, the manager did not check whether the client's concern was resolved before the client said he would think.": (
        "После сравнения со стоимостью нотариуса менеджер не проверил, снято ли сомнение клиента, прежде чем тот сказал, что подумает."
    ),
    "The call ended with the client saying she would call back, but there was no concrete next step fixed by the manager.": (
        "Звонок завершился тем, что клиент сказала, что перезвонит, но конкретный следующий шаг менеджер не закрепил."
    ),
    "Send the account creation link; registration is expected in about five minutes, followed by demo testing.": (
        "Отправить ссылку для создания аккаунта; регистрация займет около пяти минут, затем клиент протестирует демо."
    ),
    "Call once before the 10th and remind/warn again, in the context of June.": (
        "Позвонить один раз до 10-го числа и еще раз напомнить в июне."
    ),
    "Before the 10th in June context": "до 10-го числа в июне",
    "No concrete next step, owner, or date was stated; the manager only suggested possible future consideration if document volume increases.": (
        "Конкретный следующий шаг, ответственный и дата не зафиксированы; менеджер только предложил вернуться к теме, если объем документов вырастет."
    ),
    "No concrete next step, owner, or date was stated; the manager only suggested possible future consideration if document volume.": (
        "Конкретный следующий шаг, ответственный и дата не зафиксированы; менеджер только предложил вернуться к теме, если объем документов вырастет."
    ),
    "No follow-up commitment was observed; the client refused the call and subscription.": (
        "Договоренность о продолжении не зафиксирована; клиент отказался от звонка и подписки."
    ),
    "Client gave a refusal signal that electronic systems are not allowed; no next step was fixed before closing.": (
        "Клиент обозначил отказ: электронные системы не разрешены; следующий шаг перед завершением не зафиксирован."
    ),
    "Client requested an answer in WhatsApp; the call ended with a brief acceptance phrase but no explicit owner or deadline was fixed.": (
        "Клиент попросил ответить в WhatsApp; звонок завершился кратким согласием, но без явного ответственного и дедлайна."
    ),
    "The operator said they would pass the caller's questions to support.": (
        "Оператор сказал, что передаст вопросы клиента в поддержку."
    ),
    "Client was left to continue through the 'Создать' and 'Шаблон из базы' path.": (
        "Клиенту предложили продолжить через путь «Создать» и «Шаблон из базы»."
    ),
    "The client said she would call back.": "Клиент сказала, что перезвонит.",
    "The transcript contains proposed timing but does not reliably establish owner, exact due date, or speaker roles.": (
        "В расшифровке есть предложенное время, но нет надежного подтверждения ответственного, точного срока и ролей спикеров."
    ),
    "No explicit owner or timing was stated in the short transcript; speaker attribution for the closing acceptance is uncertain.": (
        "В короткой расшифровке нет явного ответственного и срока; роли спикеров в финальном согласии не подтверждены."
    ),
}

_MANAGER_DAILY_VALUE_TRANSLATIONS = {
    "manager": "Менеджер",
    "client": "Клиент",
    "evidence": "Фрагмент",
    "context": "Контекст",
...
def _localize_manager_daily_payload(payload: dict[str, Any]) -> None:
    """Best-effort Russian localization for manager-facing report text."""
    _localize_manager_daily_node(payload)


def _localize_manager_daily_node(value: Any, *, key: str = "") -> Any:
    if isinstance(value, dict):
        for item_key, item_value in list(value.items()):
            value[item_key] = _localize_manager_daily_node(item_value, key=str(item_key))
        return value
    if isinstance(value, list):
        for index, item in enumerate(list(value)):
            value[index] = _localize_manager_daily_node(item, key=key)
        return value
    if isinstance(value, str):
        if key in {"speaker", "role", "quote_role"}:
            return _MANAGER_DAILY_VALUE_TRANSLATIONS.get(value, value)
        return _localize_manager_daily_text(value)
    return value


def _localize_manager_daily_text(text: str) -> str:
    normalized = text.strip()
    if not normalized:
        return text
    if normalized in _MANAGER_DAILY_EXACT_TEXT_TRANSLATIONS:
        return _MANAGER_DAILY_EXACT_TEXT_TRANSLATIONS[normalized]

    result = text
    for source, target in sorted(
        _MANAGER_DAILY_EXACT_TEXT_TRANSLATIONS.items(),
    ...
    if lowered.startswith("the transcript contains proposed timing"):
        return (
            "В расшифровке есть предложенное время, но нет надежного подтверждения "
            "ответственного, точного срока и ролей спикеров."
        )
    if lowered.startswith("no explicit owner or timing"):
        return (
            "В короткой расшифровке нет явного ответственного и срока; роли спикеров "
            "в финальном согласии не подтверждены."
        )
    return result
...
_STAGE_FOCUS_WEIGHTS: dict[str, float] = {
    "contact_start": 0.85,
    "qualification_primary": 1.25,
    "needs_discovery": 1.40,
    "presentation": 1.30,
    "objection_handling": 1.35,
    "completion_next_step": 1.15,
    "sale_processing": 0.95,
    "sale_final": 1.00,
    "cross_stage_transition": 1.10,
}
_STAGE_FOCUS_SELECTION_METHOD = "weighted_focus_impact"
_STAGE_FOCUS_CRITICAL_LOW_SCORE_THRESHOLD = 3.0
...
def _aggregate_stage_scores(*, artifacts: list[ReportArtifact]) -> list[dict[str, Any]]:
    ...
    Priority rule: weighted focus impact. Critical low scores are shown as
    separate signals, but do not override the weighted focus.
    ...
    configured_stage_codes = {stage_code for stage_code, _funnel_label, _stage_name in _STAGE_FUNNEL_ORDER}
    stage_avgs: dict[str, float] = {}
    stage_calls: dict[str, int] = {}
    max_stage_calls = max(
        (
            len(scores)
            for stage_code, scores in stage_buckets.items()
            if stage_code in configured_stage_codes
        ),
        default=0,
    )
    for stage_code, scores in stage_buckets.items():
        if stage_code not in configured_stage_codes:
            continue
        if not scores:
            continue
        stage_avgs[stage_code] = round(sum(scores) / len(scores), 1)
        stage_calls[stage_code] = len(scores)

    focus_metrics: dict[str, dict[str, Any]] = {}
    priority_stage_code: str | None = None
    for stage_code, avg in stage_avgs.items():
        calls_count = stage_calls.get(stage_code, 0)
        stage_weight = _STAGE_FOCUS_WEIGHTS.get(stage_code, 1.0)
        coverage_factor = (
            1.0
            if max_stage_calls <= 0
            else 0.6 + 0.4 * (calls_count / max_stage_calls)
        )
        focus_impact = (10 - avg) * stage_weight * coverage_factor
        is_critical = avg < _STAGE_FOCUS_CRITICAL_LOW_SCORE_THRESHOLD
        focus_metrics[stage_code] = {
            "stage_weight": stage_weight,
            "coverage_factor": round(coverage_factor, 2),
            "focus_impact": round(focus_impact, 2),
            "priority_selection_method": _STAGE_FOCUS_SELECTION_METHOD,
            "critical_low_score_signal": is_critical,
            "critical_low_score_override": False,
        }

    if focus_metrics:
        candidate_stage_codes = list(focus_metrics)
        funnel_rank = {
            stage_code: index
            for index, (stage_code, _funnel_label, _stage_name) in enumerate(_STAGE_FUNNEL_ORDER)
        }
        priority_stage_code = max(
            candidate_stage_codes,
            key=lambda code: (
                float(focus_metrics[code].get("focus_impact") or 0.0),
                10.0 - float(stage_avgs.get(code, 10.0)),
                -funnel_rank.get(code, 999),
            ),
        )
    ...
    for stage_code, funnel_label, stage_name in _STAGE_FUNNEL_ORDER:
        avg = stage_avgs.get(stage_code)
        if avg is None:
            continue
        calls_count = stage_calls.get(stage_code, 0)
        metrics = focus_metrics.get(
            stage_code,
            {
                "stage_weight": _STAGE_FOCUS_WEIGHTS.get(stage_code, 1.0),
                "coverage_factor": 1.0,
        ...
        is_priority = stage_code == priority_stage_code
        ...
        rows.append({
            ...
            "stage_weight": metrics["stage_weight"],
            "coverage_factor": metrics["coverage_factor"],
            "focus_impact": metrics["focus_impact"],
            "priority_selection_method": metrics["priority_selection_method"],
            "critical_low_score_signal": metrics["critical_low_score_signal"],
            "critical_low_score_override": metrics["critical_low_score_override"],
...
def _build_verified_situation_day_from_daily_composer(
    *,
    ...
    prepared = _prepare_daily_situation_day_result(
        artifacts=artifacts,
        result=raw_result,
        daily_focus=daily_input.get("daily_focus") if isinstance(daily_input, dict) else daily_focus,
    )
    ...
    focus_stage_code = _daily_focus_stage_code(daily_input.get("daily_focus"))
    contract_rejection_reason = _focus_case_contract_rejection_reason(
        decision=prepared.get("focus_case_selection")
        if isinstance(prepared.get("focus_case_selection"), dict)
        else dict((prepared.get("selection_diagnostics") or {}).get("focus_case_selection") or {}),
        daily_focus=daily_input.get("daily_focus") if isinstance(daily_input, dict) else None,
        known_call_ids={str(item.interaction.id) for item in artifacts},
    )
    if contract_rejection_reason:
        prepared = dict(prepared)
        quality = dict(prepared.get("quality_diagnostics") or {})
        quality["report_layer_verification_status"] = "failed"
        quality["report_layer_rejection_reason"] = contract_rejection_reason
        prepared["quality_diagnostics"] = quality
        return (
            _no_verified_situation_day_result(
                reason=contract_rejection_reason,
                rejected=rejected,
                selection_mode="daily_situation_composer_contract",
            ),
            prepared,
            daily_input,
        )

    prepared_stage_code = str(
        (dict(prepared.get("coaching_view") or {}).get("stage_code"))
        or (dict(prepared.get("evidence_quote") or {}).get("stage_code"))
        or ""
    ).strip()
    if focus_stage_code and prepared_stage_code and prepared_stage_code != focus_stage_code:
        mismatch_reason = f"situation_stage_mismatch:{prepared_stage_code}"
        return (
            _no_verified_situation_day_result(
                reason=mismatch_reason,
                rejected=rejected,
                selection_mode="blocked_mismatch",
            ),
            prepared,
            daily_input,
        )

    proof_pool_rejection_reason = _daily_situation_proof_pool_rejection_reason(
        result=prepared,
        daily_input=daily_input,
    )
    if proof_pool_rejection_reason:
        prepared = dict(prepared)
        quality = dict(prepared.get("quality_diagnostics") or {})
        quality["report_layer_verification_status"] = "failed"
        quality["report_layer_rejection_reason"] = proof_pool_rejection_reason
        prepared["quality_diagnostics"] = quality
        return (
            _no_verified_situation_day_result(
                reason=proof_pool_rejection_reason,
                rejected=rejected,
                selection_mode="daily_situation_composer_v1",
            ),
            prepared,
    ...
    selection_diagnostics.setdefault("block", "situation_day")
    decision = dict(
        prepared.get("focus_case_selection")
        or selection_diagnostics.get("focus_case_selection")
        or {}
    )
    selection_diagnostics["focus_case_selection"] = decision
    selection_diagnostics["case_status"] = decision["case_status"]
    selection_diagnostics["evidence_level"] = decision["evidence_level"]
    selection_diagnostics["wording_mode"] = decision["wording_mode"]
    verified["selection_diagnostics"] = selection_diagnostics
    verified.setdefault("coaching_view", {})["selection_diagnostics"] = selection_diagnostics
    verified["coaching_view"] = _attach_focus_case_diagnostics(
        block=verified.get("coaching_view"),
        decision=decision,
    )
    if isinstance(prepared.get("call_breakdown"), dict):
        verified["call_breakdown"] = _attach_focus_case_diagnostics(
            block=prepared.get("call_breakdown"),
            decision=decision,
        )
...
def _prepare_daily_situation_day_result(
    *,
    ...
    focus_case_selection = _normalize_focus_case_decision_from_llm3(
        daily_focus=daily_focus,
        result=result,
        fallback_selected_call_id=selected_call_id,
        fallback_stage_code=stage_code,
    )
    ...
    if focus_case_selection:
        selection_diagnostics["focus_case_selection"] = focus_case_selection
        selection_diagnostics["case_status"] = focus_case_selection.get("case_status")
        selection_diagnostics["evidence_level"] = focus_case_selection.get("evidence_level")
        selection_diagnostics["wording_mode"] = focus_case_selection.get("wording_mode")
    coaching_view = {
        "source": SITUATION_DAY_DAILY_SOURCE,
    ...
    if focus_case_selection:
        prepared["focus_case_selection"] = focus_case_selection
    call_breakdown = _normalize_daily_llm3_call_breakdown(
        result=result,
        selected_call_ref=ref,
        selected_call_id=selected_call_id,
        focus_case_selection=focus_case_selection,
    )
    if call_breakdown is not None:
        prepared["call_breakdown"] = call_breakdown
    return prepared


def _normalize_daily_llm3_call_breakdown(
    *,
    result: dict[str, Any],
    selected_call_ref: dict[str, Any],
    selected_call_id: str,
    focus_case_selection: dict[str, Any] | None,
) -> dict[str, Any] | None:
    """Accept a manager-facing Call Breakdown only when LLM3 supplied it."""
    raw = result.get("call_breakdown")
    if not isinstance(raw, dict):
        return None
    decision = dict(focus_case_selection or {})
    if str(decision.get("case_status") or "").strip() not in {"strong", "workable"}:
        return None

    expected_stage = str(decision.get("selected_case_stage_code") or result.get("stage_code") or "").strip()
    raw_call_id = str(raw.get("call_id") or selected_call_id or "").strip()
    raw_stage = str(raw.get("stage_code") or expected_stage or "").strip()
    if raw_call_id != selected_call_id:
        return None
    if expected_stage and raw_stage and raw_stage != expected_stage:
        return None

    rows = [
        [str(cell or "").strip() for cell in row[:4]]
        for row in raw.get("rows") or []
        if isinstance(row, list) and len(row) >= 4
    ]
    if not rows and isinstance(raw.get("moments"), list):
        for index, item in enumerate(raw.get("moments") or [], start=1):
            if not isinstance(item, dict):
                continue
            moment = str(item.get("moment") or item.get("title") or f"Момент {index}").strip()
            what = str(item.get("what_happened") or item.get("what") or "").strip()
            proof = str(
                item.get("confirming_fragment")
                or item.get("evidence")
                or item.get("supporting_quote")
                or ""
            ).strip()
            better = str(item.get("better_action") or item.get("recommendation") or item.get("better") or "").strip()
            if what and better:
                rows.append([moment, what, proof, better])
    if not rows:
        return None

    summary_line = str(
        raw.get("summary_line")
        or selected_call_ref.get("client_call_reference")
        or "Разбор выбранного звонка"
    ).strip()
    selection = dict(raw.get("selection_diagnostics") or {})
    selection.update(
        {
            "selection_mode": selection.get("selection_mode") or "llm3_daily_call_breakdown",
            "selected_call_id": selected_call_id,
            "llm3_daily_call_breakdown": True,
        }
    )
    return {
        "status": "verified",
        ...
        "call_id": selected_call_id,
        "client_label": selected_call_ref.get("client_label"),
        "client_phone": selected_call_ref.get("client_phone"),
        "date_label": selected_call_ref.get("date_label"),
        "time_label": selected_call_ref.get("time_label"),
        "client_call_reference": selected_call_ref.get("client_call_reference"),
        "stage_code": raw_stage or None,
        "stage_name": raw.get("stage_name") or selected_call_ref.get("stage_name"),
        "stage_steps": [],
        "worked": [],
        "to_fix": [],
        "recommendation": None,
        "rows": rows[:4],
        "moments": list(raw.get("moments") or []),
        "summary_line": summary_line,
        "call_story": str(raw.get("call_story") or "").strip(),
        "what_manager_missed": str(raw.get("what_manager_missed") or "").strip(),
        "better_path": str(raw.get("better_path") or "").strip(),
        "dialogue_evidence": list(raw.get("dialogue_evidence") or []),
        "key_turning_points": list(raw.get("key_turning_points") or []),
        "source_note": "llm3_daily_situation.call_breakdown",
        "call_breakdown_source": "llm3_daily_situation.call_breakdown",
        "call_breakdown_evidence_strength": str(decision.get("evidence_level") or "workable"),
        "call_breakdown_fragment_present": any(str(row[2] or "").strip() for row in rows),
        "case_status": decision.get("case_status"),
        "evidence_level": decision.get("evidence_level"),
        "wording_mode": decision.get("wording_mode"),
...
def _daily_situation_proof_pool_rejection_reason(
    *,
    result: dict[str, Any] | None,
    daily_input: dict[str, Any],
) -> str | None:
    """Require Daily Situation selection to point back to verified proof-pool scenes."""
    ...
    selected_call = next(
        (
            call
            for call in daily_input.get("calls") or []
            if isinstance(call, dict) and str(call.get("call_id") or "").strip() == selected_call_id
        ),
        None,
    )
    ...
    proof_scenes = [
        scene
        for scene in selected_call.get("evidence_scenes") or []
        if isinstance(scene, dict)
        and str(scene.get("evidence_type") or "").strip()
        in {"manager_gap", "manager_coaching_moment", "stage_gap"}
        and str(scene.get("proof_status") or "").strip()
        in {"verified_proof_card", "softened_proof_card"}
    ]
    if not proof_scenes:
        return None

    raw_result = result.get("raw_daily_composer_result")
    source_owner = raw_result if isinstance(raw_result, dict) else result
    source_fact_ids = {
        str(value or "").strip()
        for value in source_owner.get("source_fact_ids") or []
        if str(value or "").strip()
    }
    if source_fact_ids:
        proof_scene_ids = {
            str(scene.get("scene_id") or "").strip()
            for scene in proof_scenes
            if str(scene.get("scene_id") or "").strip()
        }
        call_fact_ids = {
            str(value or "").strip()
            for value in selected_call.get("source_fact_ids") or []
            if str(value or "").strip()
        }
        if not source_fact_ids & (proof_scene_ids | call_fact_ids):
            return "selected_source_fact_not_in_verified_proof_pool"

    selected_texts = _daily_situation_selected_texts(result)
    if selected_texts and not any(
        _daily_situation_text_matches_proof_scene(text, scene)
        for text in selected_texts
        for scene in proof_scenes
    ):
        return "selected_quote_not_in_verified_proof_pool"

    return None


def _daily_situation_selected_texts(result: dict[str, Any]) -> list[str]:
    raw = result.get("raw_daily_composer_result") if isinstance(result.get("raw_daily_composer_result"), dict) else result
    values: list[str] = []
    for value in (
        result.get("supporting_quote"),
        (result.get("evidence_quote") or {}).get("client_text")
        if isinstance(result.get("evidence_quote"), dict)
        else None,
        (result.get("evidence_quote") or {}).get("manager_text")
        if isinstance(result.get("evidence_quote"), dict)
        else None,
        raw.get("supporting_quote") if isinstance(raw, dict) else None,
    ):
        text = _dialogue_turn_text(str(value or ""), limit=700)
        if text and text not in values:
            values.append(text)
    for value in ((raw.get("evidence_quotes") or []) if isinstance(raw, dict) else []):
        text = _dialogue_turn_text(str(value or ""), limit=700)
        if text and text not in values:
            values.append(text)
    for turn in (
        result.get("dialogue_excerpt", {}).get("turns", [])
        if isinstance(result.get("dialogue_excerpt"), dict)
        else []
    ):
        if not isinstance(turn, dict):
            continue
        text = _dialogue_turn_text(str(turn.get("text") or turn.get("quote") or ""), limit=700)
        if text and text not in values:
            values.append(text)
    return values[:12]


def _daily_situation_text_matches_proof_scene(text: str, scene: dict[str, Any]) -> bool:
    selected = _dialogue_compact(text)
    if not selected:
        return False
    scene_texts = [str(scene.get("quote") or "")]
    scene_texts.extend(
        str(turn.get("text") or turn.get("quote") or "")
        for turn in scene.get("turns") or []
        if isinstance(turn, dict)
    )
    for scene_text in scene_texts:
        compact = _dialogue_compact(scene_text)
        if not compact:
            continue
        if selected in compact or compact in selected:
            return True
        selected_tokens = {
            token for token in re.findall(r"[a-zа-яё0-9]+", selected) if len(token) >= 4
        }
        scene_tokens = {
            token for token in re.findall(r"[a-zа-яё0-9]+", compact) if len(token) >= 4
        }
        if selected_tokens and len(selected_tokens & scene_tokens) / max(len(selected_tokens), 1) >= 0.8:
            return True
    return False
...
def _registry_manager_gap_items(evidence_items: Any) -> list[dict[str, Any]]:
    ...
    for item in _registry_item_dicts(evidence_items):
        ...
        diagnostics = item.get("diagnostics") if isinstance(item.get("diagnostics"), dict) else {}
        proof_status = str(
            item.get("proof_status") or diagnostics.get("proof_status") or ""
        ).strip()
        if proof_status not in {"verified_proof_card", "softened_proof_card"}:
            continue
...
def _build_report_evidence_registry_diagnostics(
    *,
    ...
    router_diagnostics = report_block_routes.get("diagnostics") or {}
    rejected_routes = router_diagnostics.get("rejected")
    if not isinstance(rejected_routes, list):
        rejected_routes = []
    blocked_by_missing_proof_card_count = sum(
        1
        for item in rejected_routes
        if str((item or {}).get("reason") or "") == "missing_proof_card"
    )
    return {
        ...
        "router_proof_gate_mode": router_diagnostics.get("proof_gate_mode"),
        "blocked_by_missing_proof_card_count": blocked_by_missing_proof_card_count,
        "router_summary": router_diagnostics.get("summary"),
...
def _no_verified_situation_day_result(
    *,
    ...
    case_status = "blocked_mismatch" if str(reason or "").startswith("situation_stage_mismatch:") else "weak_blocked"
    decision = _focus_case_decision(
        daily_focus=None,
        case_status=case_status,
        selected_call_id=None,
...
def _final_manager_status_from_call_row(row: dict[str, Any]) -> Any:
    """Return the status actually shown to a manager for a call-list row."""
    if "final_manager_status" in row:
        return row.get("final_manager_status")
...
CALL_LIST_CONTEXT_GENERIC_PATTERNS = (
    "есть договоренность",
    "есть договорённость",
    "контакт в работу",
...
def _call_list_context_generic_reason(value: Any, *, status: str | None = None) -> str | None:
    text = re.sub(r"\s+", " ", str(value or "").strip())
    normalized = _summary_norm(text)
    if not normalized:
        return "empty_context"
    if any(pattern in normalized for pattern in CALL_LIST_CONTEXT_GENERIC_PATTERNS):
        return "generic_context"
    if status == "agreed" and normalized.startswith(("есть договоренность", "есть договорённость")):
        return "generic_agreement_context"
    if status == "refusal" and "причина отказа не извлечена" in normalized:
        return "weak_refusal_context"
    if status == "tech_service" and "продажный контекст не выявлен" in normalized:
        return "weak_service_context"
    return None
...
def _call_list_context_from_block_candidate(report_evidence: dict[str, Any] | None) -> dict[str, str] | None:
    ...
    if not isinstance(item, dict) or item.get("fit") is False:
        return None
    for field_name in ("manager_visible_summary", "call_list_context_rich", "short_context"):
        text = _call_list_context_paragraph(
            item.get(field_name),
            limit=420 if field_name != "short_context" else 280,
        )
        if text:
            return {
                "context": text,
                "source": f"report_evidence.block_candidates.call_list_context.{field_name}",
            }
    return None
...
def _call_list_context_from_semantic_case(report_evidence: dict[str, Any] | None) -> dict[str, str] | None:
    ...
    for field_name, limit in (
        ...
        text = _summary_text(semantic_case.get(field_name), limit=limit)
...
def _call_list_sentence(value: Any, *, limit: int = 220) -> str | None:
    text = _summary_text(value, limit=limit)
    if not text:
        return None
    text = text.strip().rstrip(".")
    if not text or _call_list_context_reject_reason(text):
        return None
    return text


def _append_unique_call_list_part(parts: list[str], value: str | None) -> None:
    text = str(value or "").strip()
    if not text:
        return
    normalized = _summary_norm(text)
    if not normalized:
        return
    existing = " ".join(_summary_norm(part) for part in parts)
    if normalized in existing:
        return
    parts.append(text if text.endswith((".", "!", "?")) else f"{text}.")


def _call_list_outcome_essence_context(
    *,
    status: str | None,
    call_type: str | None,
    ...
    evidence_follow_up_reason: str | None,
) -> dict[str, Any] | None:
    """Build a concrete manager-facing call essence from LLM-2 outcome/follow-up fields."""
    ...
    topic = (
        _call_list_sentence(summary_topic, limit=120)
        if summary_topic and _call_summary_topic_usable(summary_topic)
        else None
    )
    outcome = _call_list_sentence(outcome_reason, limit=220)
    evidence = _call_list_sentence(outcome_evidence, limit=180)
    follow_up_reason = _call_list_sentence(evidence_follow_up_reason or reason, limit=180)
    action = _call_list_sentence(next_step, limit=180)
    signal = _call_list_sentence(signal_text, limit=180)
    source = "report_evidence.business_outcome.call_essence"
    parts: list[str] = []

    if status_text == "refusal":
        refusal_reason = outcome or follow_up_reason or evidence
        if not refusal_reason:
            return None
        _append_unique_call_list_part(parts, f"Клиент отказался: {refusal_reason}")
    elif status_text == "tech_service" or call_type_text in {"support", "internal"}:
        issue = outcome or evidence or signal
        if not issue:
            if scenario_text == "after_signed_document":
                issue = "вопрос после подписания документа"
            else:
                return None
        _append_unique_call_list_part(parts, f"Сервисный вопрос: {issue}")
    elif status_text in {"agreed", "rescheduled"}:
        if topic:
            _append_unique_call_list_part(parts, f"Тема: {topic}")
        agreement = outcome or follow_up_reason or evidence
        if not agreement and action:
            agreement = action
        if agreement:
            label = "Перенос" if status_text == "rescheduled" else "Договорённость"
            deadline_suffix = f"; когда: {deadline_phrase}" if deadline_phrase else ""
            _append_unique_call_list_part(parts, f"{label}: {agreement}{deadline_suffix}")
        if action and _summary_norm(action) not in _summary_norm(agreement or ""):
            _append_unique_call_list_part(parts, f"Дальше: {action}")
            source = "report_evidence.follow_up_candidates.follow_up_essence"
        if deadline_phrase and not agreement:
            _append_unique_call_list_part(parts, f"Когда: {deadline_phrase}")
        if not parts or (status_text == "agreed" and not (agreement or topic or action)):
            return None
    elif status_text == "open":
        open_reason = outcome or follow_up_reason or evidence
        if action:
            _append_unique_call_list_part(parts, f"Дальше: {action}")
            source = "report_evidence.follow_up_candidates.follow_up_essence"
        if open_reason:
            _append_unique_call_list_part(parts, f"Контакт открыт: {open_reason}")
        if deadline_phrase:
            _append_unique_call_list_part(parts, f"Когда: {deadline_phrase}")
        if not parts:
            return None
    ...
    return {
        ...
        "diagnostics": {
            ...
            "has_outcome_reason": bool(outcome),
            "has_next_step": bool(action),
            "has_deadline": bool(deadline_phrase),
        },
    }


def _call_list_context_from_call_essence(
    ...
    *,
    ...
    if not isinstance(report_evidence, dict):
        return None
    essence = report_evidence.get("call_essence")
    if not isinstance(essence, dict):
        return None
    status_text = str(status or essence.get("outcome") or "").strip()
    source_name = str(essence.get("source") or "call_essence").strip()
    explicit = source_name != "derived_from_existing_report_evidence"
    manager_visible = _call_list_context_paragraph(essence.get("manager_visible_text"), limit=640)
    if manager_visible and not _call_list_context_generic_reason(manager_visible, status=status_text):
        return {
            "context": manager_visible,
            "source": "report_evidence.call_essence.manager_visible_text",
            "explicit": explicit,
            "diagnostics": {
                "status": "passed",
                "explicit": explicit,
...
def _call_list_status_fallback_context(
    *,
    ...
    scenario_text = str(scenario_type or "").strip()
    deadline_phrase = _call_list_deadline_phrase(deadline)
    next_step_text = _call_list_context_text(next_step, limit=220)
    reason_text = _call_list_context_text(reason, limit=220)
    ...
    if status_text == "tech_service" or call_type_text in {"support", "internal"}:
        issue = reason_text or next_step_text
        if issue:
            return f"Сервисный вопрос: {issue.rstrip('.')}."
        if scenario_text == "after_signed_document":
    ...
    if status_text == "rescheduled":
        if reason_text and deadline_phrase:
            return f"Перенос: {reason_text.rstrip('.')}. Вернуться {deadline_phrase}."
        if next_step_text and deadline_phrase:
            return f"Перенос: {next_step_text.rstrip('.')}. Срок: {deadline_phrase}."
        if reason_text or next_step_text:
            return f"Перенос: {(reason_text or next_step_text).rstrip('.')}."
        if deadline_phrase:
    ...
    if status_text == "agreed":
        if next_step_text and deadline_phrase:
            return f"Договорились: {next_step_text.rstrip('.')}. Срок: {deadline_phrase}."
        if next_step_text:
            return f"Договорились: {next_step_text.rstrip('.')}."
        if reason_text:
            suffix = f" Срок: {deadline_phrase}." if deadline_phrase else ""
            return f"Договорённость: {reason_text.rstrip('.')}.{suffix}"
    ...
    if status_text == "open":
        if next_step_text:
            return f"Открытый контакт: {next_step_text.rstrip('.')}."
        if reason_text:
            return f"Открытый контакт: {reason_text.rstrip('.')}."
...
def _select_call_list_context(
    *,
    ...
    def reject(source: str, value: Any) -> str | None:
        reason_code = _call_list_context_reject_reason(value) or _call_list_context_generic_reason(
            value,
            status=status,
        )
        if reason_code:
    ...
    if summary_manager_visible and not reject(
        ...
        return {
            ...
            "priority": 20,
            "selected_reason": "manager_visible_summary_selected",
            "rejected": rejected,
            "fallback_generated": False,
            "bare_context_retained": False,
        }

    call_essence_context = _call_list_context_from_call_essence(
        report_evidence,
        status=status,
        deadline=deadline,
    )
    if (
        call_essence_context
        and call_essence_context.get("explicit")
        and not reject(call_essence_context["source"], call_essence_context["context"])
    ):
        return {
            "context": call_essence_context["context"],
            "source": call_essence_context["source"],
            "priority": 10,
            "selected_reason": "explicit_call_essence_selected",
            "rejected": rejected,
            "fallback_generated": False,
            "bare_context_retained": False,
    ...
    for rich_candidate in (
        ...
        if rich_candidate and not reject(rich_candidate["source"], rich_candidate["context"]):
            return {
                ...
                "priority": 30,
                "selected_reason": "rich_context_selected",
                "rejected": rejected,
                "fallback_generated": False,
                "bare_context_retained": False,
            }
    ...
    if summary_context and _call_summary_context_usable(summary_context) and not reject(
        ...
        return {
            ...
            "priority": 40,
            "selected_reason": "short_context_selected",
            "rejected": rejected,
            "fallback_generated": False,
            "bare_context_retained": False,
        }
    ...
    if summary_topic and _call_summary_topic_usable(summary_topic) and not reject(
        ...
        return {
            ...
            "priority": 50,
            "selected_reason": "short_topic_selected",
            "rejected": rejected,
            "fallback_generated": False,
            "bare_context_retained": False,
        }
...
def _build_call_list_context_quality_diagnostics(call_list: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in call_list:
        ...
        context_diagnostics = dict(row.get("call_list_context_diagnostics") or {})
        weak_reason = (
            str(context_diagnostics.get("weak_reason") or "").strip()
            or _call_list_context_generic_reason(row.get("call_list_context"), status=final_status)
        )
        row_quality = dict(row.get("call_list_context_quality") or {})
        row_diagnostics.append(
            {
                ...
                "rejected_count": row_quality.get("rejected_count", 0),
            }
        )
        if weak_reason:
            weak_rows.append(
    ...
    return {
        ...
        "call_essence_context_count": sum(
            count for source, count in source_counts.items()
            if source.startswith("report_evidence.call_essence")
        ),
        "outcome_essence_context_count": source_counts.get("report_evidence.business_outcome.call_essence", 0),
        "follow_up_essence_context_count": source_counts.get("report_evidence.follow_up_candidates.follow_up_essence", 0),
        ...
        "weak_context_count": len(weak_rows),
        "weak_contexts": weak_rows[:20],
            ...
            conflicts.append(
                {
                    ...
                    "final_manager_status": _final_manager_status_from_call_row(row),
...
def _build_agreement_outcome_diagnostics(call_list: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in call_list:
        ...
        display_status = _final_manager_status_from_call_row(row)
...
def _build_daily_call_row(
    ...
    *,
    ...
    summary_next_action = _summary_text((summary or {}).get("manager_next_action"), limit=240) if summary else None
    ...
    evidence_follow_up = _select_report_evidence_follow_up(
        interaction_id=str(artifact.interaction.id),
        final_status=str(call_list_status or ""),
        report_evidence_index=report_evidence_index,
    )
    context_next_step = str(
        (summary_next_action if _call_summary_action_usable(summary_next_action) else None)
        or (evidence_follow_up or {}).get("next_step")
        or next_step
        or ""
    ).strip() or None
    context_deadline = str(
        (evidence_follow_up or {}).get("deadline")
        or deadline
        or follow_up.get("due_date_text")
        or follow_up.get("due_date_iso")
        or ""
    ).strip() or None
    llm2_outcome_reason = _summary_text((llm2_business_outcome or {}).get("reason"), limit=280)
    llm2_outcome_evidence = _summary_text((llm2_business_outcome or {}).get("evidence_quote"), limit=220)
    evidence_follow_up_reason = _summary_text((evidence_follow_up or {}).get("why_follow_up"), limit=220)
    ...
    context_diagnostics = dict(context_selection.get("diagnostics") or {})
    weak_reason = _call_list_context_generic_reason(visible_context, status=call_list_status)
    if weak_reason and "weak_reason" not in context_diagnostics:
        context_diagnostics["status"] = "weak"
        context_diagnostics["weak_reason"] = weak_reason
    ...
    return {
        ...
        "next_step": next_step,
        "call_list_next_step": context_next_step,
        "deadline": deadline,
        ...
        "call_report_summary_manager_next_action": summary_next_action,
        "call_list_evidence_follow_up_reason": evidence_follow_up_reason,
        ...
        "call_list_context_quality": {
            ...
            "rejected_sources": [
                ...
                for item in context_selection["rejected"]
            ],
        },
        "call_list_context_diagnostics": context_diagnostics,
...
def _is_analysis_reusable_for_reporting(
    ...
    *,
    ...
    if (
        settings.ai_llm2_semantic_validation_enabled
        and not (detail.get("score_by_stage") or [])
...
def _build_report_evidence_index(*, artifacts: list[ReportArtifact]) -> dict[str, dict[str, Any]]:
    ...
    for artifact in artifacts:
        ...
        if not valid and isinstance(raw_report_evidence, dict):
            soft_call_essence = raw_report_evidence.get("call_essence")
            if isinstance(soft_call_essence, dict) and soft_call_essence:
                report_evidence["call_essence"] = dict(soft_call_essence)
        if block_candidates:
...
def _valid_call_report_summary_for_interaction(
    *,
    ...
    raw_evidence = entry.get("report_evidence")
    evidence = (
        raw_evidence
...
def _apply_call_breakdown_quality_gate(
    *,
    ...
    if focus_stage and block_stage and block_stage != focus_stage:
        ...
        quality["status"] = "blocked_mismatch"
        quality["render_block"] = False
        quality["blocker_reason"] = f"call_breakdown_stage_mismatch:{block_stage}"
        quality["focus_stage_code"] = focus_stage
        quality["call_breakdown_stage_code"] = block_stage
        result, quality = _call_breakdown_quality_fallback(call_breakdown=call_breakdown, quality=quality)
        result["case_status"] = "blocked_mismatch"
        result["evidence_level"] = "weak"
        result["wording_mode"] = "blocked"
        result["selection_reason"] = "call_breakdown_stage_mismatch"
        ...
        diagnostics.update(
            {
                ...
                "evidence_level": "weak",
                "wording_mode": "blocked",
                "selection_reason": "call_breakdown_stage_mismatch",
                "rejection_reasons": [quality["blocker_reason"]],
            }
        )
...
def _build_report_evidence_situation(
    *,
    ...
    if not selected_block_candidates and block_relaxed_candidates and not focus_stage_code:
    ...
    if not selected_semantic_candidates and semantic_relaxed_candidates and not focus_stage_code:
    ...
    *,
    ...
    for artifact in artifacts:
        ...
        for item in evidence.get("additional_situations") or []:
            ...
            if quality not in {"direct", "indirect"} or priority not in {"high", "medium"}:
                continue
...
def _build_call_tomorrow(
    *,
    ...
    for row in call_list:
        ...
        row_context_diagnostics = dict(row.get("call_list_context_diagnostics") or {})
        row_context_weak_reason = (
            str(row_context_diagnostics.get("weak_reason") or "").strip()
            or _call_list_context_reject_reason(row_context)
        ...
        raw_reason = str(
            (aligned_summary_context if _call_summary_context_usable(aligned_summary_context) else None)
            or (aligned_hotness_reason if _call_summary_action_usable(aligned_hotness_reason) else None)
        ...
        profile = _call_tomorrow_profile(
            ...
            summary_context=aligned_summary_context,
            summary_hotness_reason=aligned_hotness_reason,
            evidence_follow_up=evidence_follow_up,
            row=row,
        )
        ...
        candidate_item = {
            ...
            "final_manager_status_source": (
                row.get("final_manager_status_source") or row.get("call_list_status_source")
            ),
    ...
    for priority_code in priority_order:
        for item in sorted(grouped[priority_code], key=lambda x: x["_sort_key"]):
            ...
            contacts.append({
                ...
                "final_manager_status_source": item["final_manager_status_source"],
